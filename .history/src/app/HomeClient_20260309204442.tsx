...
export default function HomeClient({ initialHasEntries }: { initialHasEntries: boolean }) {
  ...
  async function playAndFetch() {
    ...
    setTimeout(() => { cooldownRef.current = false }, SHAKE_COOLDOWN_MS)
  ...
  useEffect(() => { playRef.current = playAndFetch })
  ...
  useEffect(() => {
    if (motionPerm !== 'granted' || typeof DeviceMotionEvent === 'undefined') return
  ...
  return (
    <main className="min-h-screen bg-bg-base flex flex-col select-none">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-4 pb-4">
        ...
        <div className="flex items-end justify-center" style={{ minHeight: '180px' }}>
        ...
        <motion.div
          ...
          whileHover={canTrigger ? { scale: 1.05 } : {}}
