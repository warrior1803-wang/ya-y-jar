...
export interface SlipCardProps {
  ...
  onReset?: () => void;
  /** Start fully open with no animation (for static display) */
  defaultOpen?: boolean;
  /** Hide the 再抽一张 reset button */
  hideReset?: boolean;
}
...
export default function SlipCard({
  ...
  const { t } = useLang();
  const [phase, setPhase] = useState<Phase>(defaultOpen ? "open" : "folded");
  const phaseRef = useRef<Phase>(defaultOpen ? "open" : "folded");
  const containerCtrl = useAnimation();
  const textCtrl = useAnimation();
  // defaultOpen cards are SSR-rendered — skip random to avoid hydration mismatch
  const [rotation] = useState(() => defaultOpen ? "0" : ((Math.random() * 2 - 1)).toFixed(2));
  ...
  useEffect(() => {
    if (!triggered) { triggeredHandled.current = false; return; }
    if (triggeredHandled.current) return;
    triggeredHandled.current = true;
    // Defer one tick so this isn't a synchronous setState-in-effect
    const id = setTimeout(() => startAnimation(), 0);
  ...
  return (
    <motion.div
      ...
      initial={
        defaultOpen
          ? { width: "min(320px, calc(100vw - 32px))", height: "auto", rotate: parseFloat(rotation) }
    ...
    >
      ...
      >
        <motion.div animate={textCtrl} initial={defaultOpen ? { opacity: 1, y: 0 } : { opacity: 0, y: 8 }}>
          {/* Date — top right */}
          <p
            style={{
              textAlign: "right",
