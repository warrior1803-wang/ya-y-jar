...
export default function HomeClient({
  ...
  let caption = t("home.tapHint");
  if (motionPerm === "granted") caption = t("home.shakeHint");
  if (!hasEntries) caption = t("home.emptyCaption");
  if (jarState === "fetching") caption = t("home.fetchingCaption");

  return (
    <main className="min-h-screen bg-bg-base flex flex-col select-none">
      {/* ── Main content ── */}
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-4 pb-20">
        ...
        {isIOS && motionPerm === "unknown" && hasEntries && (
          <button
            ...
            className="text-sm text-text-secondary border border-border rounded-sm px-4 py-3 hover:bg-bg-accent transition-colors"
            style={{ fontFamily: FONT }}
          >
            {t("home.motionBtn")}
          </button>
